# app/wav2lip_inference_logic.py

import numpy as np
import scipy, cv2, os, sys, audio # audio.py needs to be in the PYTHONPATH or same directory
...
# Assuming face_detection.py and models.py (containing Wav2Lip) are accessible
# You might need to adjust imports based on your project structure if Wav2Lip code is in a subfolder
# e.g., from Wav2Lip_master.face_detection import FaceAlignment
# e.g., from Wav2Lip_master.models import Wav2Lip
import face_detection # Make sure face_detection.py is in your project
from models import Wav2Lip # Make sure models.py (Wav2Lip class) is in your project
# --- Configuration (can be adjusted or passed as parameters) ---
mel_step_size = 16
img_size_global = 96 # Default img_size, can be overridden by function parameter
...
def detect_faces(images, face_det_batch_size, pads, nosmooth):
    # Initialize face detector. Ideally, this should be initialized once if called multiple times.
    # For simplicity here, it's initialized per call.
    ...
    while True:
        predictions = []
        try:
            for i in range(0, len(images), batch_size): # Use tqdm for progress if you prefer
    ...
    for rect, image in zip(predictions, images):
        if rect is None:
            # cv2.imwrite('temp/faulty_frame.jpg', image) # Consider logging or handling this
...
def datagen_for_wav2lip(full_frames, mels, box_coords, static_mode, img_size_param, wav2lip_batch_size_param,
    ...
    if box_coords[0] == -1: # If no specific bounding box is provided, detect faces
        if not static_mode:
    ...
    else: # Use provided bounding box
    ...
    for i, m in enumerate(mels):
        ...
        face_crop, coords = face_crops_with_coords[idx] # Use the already detected/cropped face
        ...
        if len(img_batch) >= wav2lip_batch_size_param:
            ...
            img_masked[:, img_size_param//2:] = 0 # Mask bottom half

            # Concatenate masked and original face crops, normalize
    ...
    if len(img_batch) > 0: # Process any remaining batches
        img_batch_np, mel_batch_np = np.asarray(img_batch), np.asarray(mel_batch)
        img_masked = img_batch_np.copy()
...
def run_wav2lip_inference(
    ...
    rotate: bool = False,
    nosmooth: bool = False,
    img_size: int = img_size_global, # Use global or pass as param
    temp_dir: str = "app/temp" # Directory for temporary files
):
    """
    ...
    temp_video_path = os.path.join(temp_dir, "result.avi") # Intermediate video without audio

    # Determine if input face is a static image
    if os.path.isfile(face_input_path) and face_input_path.lower().split('.')[-1] in ['jpg', 'png', 'jpeg']:
        static_mode_internal = True
    else:
        static_mode_internal = static # Respect the static flag if it's a video
    ...
    # --- Load Audio and Create Mel Spectrograms ---
    ...
    # Synchronize video frames with mel chunks
    # (Original script slices full_frames based on mel_chunks length)
    if len(full_frames) > len(mel_chunks):
        if static_mode_internal: # If static, replicate the single frame
            full_frames = [full_frames[0] for _ in range(len(mel_chunks))]
        else: # If video, truncate or loop (here, truncating)
            full_frames = full_frames[:len(mel_chunks)]
    elif len(mel_chunks) > len(full_frames) and full_frames: # If mels are more, replicate last frame
         if static_mode_internal:
            full_frames = [full_frames[0] for _ in range(len(mel_chunks))]
         else:
    ...
    # Use the first available frame to get dimensions if full_frames is not empty
    ...
    # The model should be loaded once outside this function for server applications.
    # model = load_wav2lip_model(checkpoint_path) # Moved to be passed as argument
    model = wav2lip_model_instance # Use the pre-loaded model
    ...
    for i_batch, (img_batch_processed, mel_batch_processed, original_frames_batch, coords_batch) in enumerate(
        ...
        for predicted_face, original_frame, coord_info in zip(predicted_faces_np, original_frames_batch, coords_batch):
            ...
            # Place the lip-synced face back onto the original frame
            final_frame = original_frame.copy() # Ensure we are modifying a copy
    ...
    # Use processed_audio_path which is guaranteed to be .wav
    ...
    # --- Cleanup Temporary Files (optional, depends on your workflow) ---
    # if os.path.exists(temp_audio_path) and processed_audio_path == temp_audio_path :
